    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ->children()
                ...
                ->scalarNode('api_key')
                    ->defaultValue('')
                    ->info('Secret server credential (X-Api-Key for POST /api/v1/errors). SERVER-side only; NEVER injected into the browser. Empty = bundle stays inert.')
                ->end()
                ->scalarNode('publishable_key')
                    ->defaultValue('')
                    ->info('World-readable browser publishable key (pk_<env>_<hex>) inlined into the JS SDK init script and sent as X-Publishable-Key / ?pk= on browser ingest. NOT a secret. Empty = JS SDK config invalid (no browser ingest).')
                ->end()
                ->booleanNode('enabled')
                    ->defaultTrue()
                ...
                ->scalarNode('log_path')
                    ...
                    ->setDeprecated('applogger/symfony-bundle', '2.0', 'The "%node%" option is deprecated and ignored since the bundle delegates to applogger/sdk-core.')
                ...
                ->arrayNode('circuit_breaker')
                    ...
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultTrue()
                            ->setDeprecated('applogger/symfony-bundle', '2.0', 'The "%node%" option is deprecated and ignored since the bundle delegates to applogger/sdk-core (its circuit breaker is always-on).')
                ...
                // SDK-core wiring
                ->arrayNode('loopback_paths')
                    ->prototype('scalar')->end()
                    ->defaultValue(['/api/v1/errors', '/api/v1/js-errors', '/api/v1/sessions', '/api/v1/logs', '/api/errors'])
                    ->info('Request path prefixes treated as ingestion routes for the same-host loopback guard.')
                ->end()
                ->scalarNode('session_hash_salt')
                    ->defaultNull()
                    ->info('Explicit salt for session-hash keying; avoids the DSN-derived fallback so session continuity survives.')
                ->end()
            ->end()
        ;
